const QRCodeLimitLength = [
  ...
  [53, 42, 32, 24], [78, 62, 46, 34], [106, 84, 60, 44], [134, 106, 74, 58], [154, 122, 86, 64], [192, 152, 108, 84],
  [230, 180, 130, 98], [271, 213, 151, 119], [321, 251, 177, 137], [367, 287, 203, 155], [425, 331, 241, 177], [458, 362, 258, 194],
  [520, 412, 292, 220], [586, 450, 322, 250], [644, 504, 364, 280], [718, 560, 394, 310], [792, 624, 442, 338], [858, 666, 482, 382],
  ...
  setupPositionProbePattern(row, col) {
    for (let r = -1; r <= 7; r++) {
      ...
      for (let c = -1; c <= 7; c++) {
        ...
        if ((r >= 0 && r <= 6 && (parseInt(c, 10) === 0 || parseInt(c, 10) === 6)) || (c >= 0 && c <= 6 && (parseInt(r, 10) === 0 || parseInt(r, 10) === 6)) || (r >= 2 && r <= 4 && c >= 2 && c <= 4)) {
...
QRCodeModel.createData = function createData(typeNumber, errorCorrectLevel, dataList) {
...
QRCodeModel.createBytes = function createBytes(buffer, rsBlocks) {
...
QRRSBlock.getRsBlockTable = function getRsBlockTable(typeNumber, errorCorrectLevel) { switch (errorCorrectLevel) { case QRErrorCorrectLevel.L: return QRRSBlock.RS_BLOCK_TABLE[(typeNumber - 1) * 4 + 0]; case QRErrorCorrectLevel.M: return QRRSBlock.RS_BLOCK_TABLE[(typeNumber - 1) * 4 + 1]; case QRErrorCorrectLevel.Q: return QRRSBlock.RS_BLOCK_TABLE[(typeNumber - 1) * 4 + 2]; case QRErrorCorrectLevel.H: return QRRSBlock.RS_BLOCK_TABLE[(typeNumber - 1) * 4 + 3]; default: return undefined; } };
...
const QRUtil = {
  ...
  getBCHDigit(data) {
    let digit = 0; while (parseInt(data, 10) !== 0) { digit++; data >>>= 1; }
    return digit;
  },
  ...
  getMask(maskPattern, i, j) {
    switch (maskPattern) {
      ...
      case QRMaskPattern.PATTERN111: return (((i * j) % 3) + ((i + j) % 2)) % 2 === 0; default:
...
function QRCode($options = {}) {
  ...
  const options = typeof $options === 'string' ? { text: $options } : $options;
  Object.assign(this._options, options);

  if (!(this._options.text && this._options.canvasId)) return;
  this._oQRCode = null;
  this.makeCode(this._options.text);
}

QRCode.prototype.makeCode = function makeCode(sText) {
  this._oQRCode = new QRCodeModel(_getTypeNumber(sText, this._options.correctLevel), this._options.correctLevel);
  this._oQRCode.addData(sText);
  this._oQRCode.make();
  this.makeImage();
};

QRCode.prototype.makeImage = function makeImage() {
  let _oContext;
  if (this._options.usingIn) {
    _oContext = wx.createCanvasContext(this._options.canvasId, this._options.usingIn);
  } else {
    _oContext = wx.createCanvasContext(this._options.canvasId);
  }
  const { _options } = this;
  const oQRCode = this._oQRCode;

  const nCount = oQRCode.getModuleCount();
  const nWidth = _options.padding ? (_options.width - 2 * _options.padding) / nCount : _options.width / nCount;
  const nHeight = _options.padding ? (_options.height - 2 * _options.padding) / nCount : _options.height / nCount;
  const nRoundedHeight = Math.round(nHeight);
  const nRoundedWidth = Math.round(nWidth);

  _oContext.setFillStyle('#fff');
  _oContext.fillRect(0, 0, _options.width, _options.height);
  _oContext.save();
  for (let row = 0; row < nCount; row++) {
    for (let col = 0; col < nCount; col++) {
      const bIsDark = oQRCode.isDark(row, col);
      const nLeft = _options.padding ? col * nWidth + _options.padding : col * nWidth;
      const nTop = _options.padding ? row * nHeight + _options.padding : row * nHeight;
      _oContext.setStrokeStyle(bIsDark ? _options.fontColor : _options.backColor);
      // _oContext.setStrokeStyle('red')
      _oContext.setLineWidth(1);
      _oContext.setFillStyle(bIsDark ? _options.fontColor : _options.backColor);
  ...
  if (_options.image && typeof _options.image === 'object') {
    const dx = _options.width / 2 - _options.image.destWidth / 2;
    const dy = _options.height / 2 - _options.image.destHeight / 2;
    _oContext.drawImage(_options.image.imageResource, _options.image.dx || dx, _options.image.dy || dy, _options.image.destWidth, _options.image.destHeight);
  }
...
// 保存为图片，将临时路径传给回调
QRCode.prototype.exportImage = function exportImage() {
  if (!(this._options.callback && typeof this._options.callback === 'function')) return;
  const options = {
    x: 0,
    y: 0,
    width: this._options.width,
    height: this._options.height,
    destWidth: this._options.width,
    destHeight: this._options.height,
    canvasId: this._options.canvasId,
    success: this._options.callback,
  };
  wx.canvasToTempFilePath(options, this._options.usingIn);
};
