const CHAR_TILDE = 126;
const CODE_FNC1 = 102;

const SET_STARTA = 103;
const SET_STARTB = 104;
const SET_STARTC = 105;
const SET_SHIFT = 98;
const SET_CODEA = 101;
const SET_CODEB = 100;
const SET_STOP = 106;


const REPLACE_CODES = {
  CHAR_TILDE: CODE_FNC1, // ~ corresponds to FNC1 in GS1-128 standard
};

const CODESET = {
  ANY: 1,
  AB: 2,
  A: 3,
  B: 4,
  C: 5,
};

const PATTERNS = [
  [2, 1, 2, 2, 2, 2, 0, 0], // 0
  [2, 2, 2, 1, 2, 2, 0, 0], // 1
  [2, 2, 2, 2, 2, 1, 0, 0], // 2
  [1, 2, 1, 2, 2, 3, 0, 0], // 3
  [1, 2, 1, 3, 2, 2, 0, 0], // 4
  [1, 3, 1, 2, 2, 2, 0, 0], // 5
  [1, 2, 2, 2, 1, 3, 0, 0], // 6
  [1, 2, 2, 3, 1, 2, 0, 0], // 7
  [1, 3, 2, 2, 1, 2, 0, 0], // 8
  [2, 2, 1, 2, 1, 3, 0, 0], // 9
  [2, 2, 1, 3, 1, 2, 0, 0], // 10
  [2, 3, 1, 2, 1, 2, 0, 0], // 11
  [1, 1, 2, 2, 3, 2, 0, 0], // 12
  [1, 2, 2, 1, 3, 2, 0, 0], // 13
  [1, 2, 2, 2, 3, 1, 0, 0], // 14
  [1, 1, 3, 2, 2, 2, 0, 0], // 15
  [1, 2, 3, 1, 2, 2, 0, 0], // 16
  [1, 2, 3, 2, 2, 1, 0, 0], // 17
  [2, 2, 3, 2, 1, 1, 0, 0], // 18
  [2, 2, 1, 1, 3, 2, 0, 0], // 19
  [2, 2, 1, 2, 3, 1, 0, 0], // 20
  [2, 1, 3, 2, 1, 2, 0, 0], // 21
  [2, 2, 3, 1, 1, 2, 0, 0], // 22
  [3, 1, 2, 1, 3, 1, 0, 0], // 23
  [3, 1, 1, 2, 2, 2, 0, 0], // 24
  [3, 2, 1, 1, 2, 2, 0, 0], // 25
  [3, 2, 1, 2, 2, 1, 0, 0], // 26
  [3, 1, 2, 2, 1, 2, 0, 0], // 27
  [3, 2, 2, 1, 1, 2, 0, 0], // 28
  [3, 2, 2, 2, 1, 1, 0, 0], // 29
  [2, 1, 2, 1, 2, 3, 0, 0], // 30
  [2, 1, 2, 3, 2, 1, 0, 0], // 31
  [2, 3, 2, 1, 2, 1, 0, 0], // 32
  [1, 1, 1, 3, 2, 3, 0, 0], // 33
  [1, 3, 1, 1, 2, 3, 0, 0], // 34
  [1, 3, 1, 3, 2, 1, 0, 0], // 35
  [1, 1, 2, 3, 1, 3, 0, 0], // 36
  [1, 3, 2, 1, 1, 3, 0, 0], // 37
  [1, 3, 2, 3, 1, 1, 0, 0], // 38
  [2, 1, 1, 3, 1, 3, 0, 0], // 39
  [2, 3, 1, 1, 1, 3, 0, 0], // 40
  [2, 3, 1, 3, 1, 1, 0, 0], // 41
  [1, 1, 2, 1, 3, 3, 0, 0], // 42
  [1, 1, 2, 3, 3, 1, 0, 0], // 43
  [1, 3, 2, 1, 3, 1, 0, 0], // 44
  [1, 1, 3, 1, 2, 3, 0, 0], // 45
  [1, 1, 3, 3, 2, 1, 0, 0], // 46
  [1, 3, 3, 1, 2, 1, 0, 0], // 47
  [3, 1, 3, 1, 2, 1, 0, 0], // 48
  [2, 1, 1, 3, 3, 1, 0, 0], // 49
  [2, 3, 1, 1, 3, 1, 0, 0], // 50
  [2, 1, 3, 1, 1, 3, 0, 0], // 51
  [2, 1, 3, 3, 1, 1, 0, 0], // 52
  [2, 1, 3, 1, 3, 1, 0, 0], // 53
  [3, 1, 1, 1, 2, 3, 0, 0], // 54
  [3, 1, 1, 3, 2, 1, 0, 0], // 55
  [3, 3, 1, 1, 2, 1, 0, 0], // 56
  [3, 1, 2, 1, 1, 3, 0, 0], // 57
  [3, 1, 2, 3, 1, 1, 0, 0], // 58
  [3, 3, 2, 1, 1, 1, 0, 0], // 59
  [3, 1, 4, 1, 1, 1, 0, 0], // 60
  [2, 2, 1, 4, 1, 1, 0, 0], // 61
  [4, 3, 1, 1, 1, 1, 0, 0], // 62
  [1, 1, 1, 2, 2, 4, 0, 0], // 63
  [1, 1, 1, 4, 2, 2, 0, 0], // 64
  [1, 2, 1, 1, 2, 4, 0, 0], // 65
  [1, 2, 1, 4, 2, 1, 0, 0], // 66
  [1, 4, 1, 1, 2, 2, 0, 0], // 67
  [1, 4, 1, 2, 2, 1, 0, 0], // 68
  [1, 1, 2, 2, 1, 4, 0, 0], // 69
  [1, 1, 2, 4, 1, 2, 0, 0], // 70
  [1, 2, 2, 1, 1, 4, 0, 0], // 71
  [1, 2, 2, 4, 1, 1, 0, 0], // 72
  [1, 4, 2, 1, 1, 2, 0, 0], // 73
  [1, 4, 2, 2, 1, 1, 0, 0], // 74
  [2, 4, 1, 2, 1, 1, 0, 0], // 75
  [2, 2, 1, 1, 1, 4, 0, 0], // 76
  [4, 1, 3, 1, 1, 1, 0, 0], // 77
  [2, 4, 1, 1, 1, 2, 0, 0], // 78
  [1, 3, 4, 1, 1, 1, 0, 0], // 79
  [1, 1, 1, 2, 4, 2, 0, 0], // 80
  [1, 2, 1, 1, 4, 2, 0, 0], // 81
  [1, 2, 1, 2, 4, 1, 0, 0], // 82
  [1, 1, 4, 2, 1, 2, 0, 0], // 83
  [1, 2, 4, 1, 1, 2, 0, 0], // 84
  [1, 2, 4, 2, 1, 1, 0, 0], // 85
  [4, 1, 1, 2, 1, 2, 0, 0], // 86
  [4, 2, 1, 1, 1, 2, 0, 0], // 87
  [4, 2, 1, 2, 1, 1, 0, 0], // 88
  [2, 1, 2, 1, 4, 1, 0, 0], // 89
  [2, 1, 4, 1, 2, 1, 0, 0], // 90
  [4, 1, 2, 1, 2, 1, 0, 0], // 91
  [1, 1, 1, 1, 4, 3, 0, 0], // 92
  [1, 1, 1, 3, 4, 1, 0, 0], // 93
  [1, 3, 1, 1, 4, 1, 0, 0], // 94
  [1, 1, 4, 1, 1, 3, 0, 0], // 95
  [1, 1, 4, 3, 1, 1, 0, 0], // 96
  [4, 1, 1, 1, 1, 3, 0, 0], // 97
  [4, 1, 1, 3, 1, 1, 0, 0], // 98
  [1, 1, 3, 1, 4, 1, 0, 0], // 99
  [1, 1, 4, 1, 3, 1, 0, 0], // 100
  [3, 1, 1, 1, 4, 1, 0, 0], // 101
  [4, 1, 1, 1, 3, 1, 0, 0], // 102
  [2, 1, 1, 4, 1, 2, 0, 0], // 103
  [2, 1, 1, 2, 1, 4, 0, 0], // 104
  [2, 1, 1, 2, 3, 2, 0, 0], // 105
  [2, 3, 3, 1, 1, 1, 2, 0], // 106
];

function getBytes(str) {
  const bytes = [];
  for (let i = 0; i < str.length; i++) {
    bytes.push(str.charCodeAt(i));
  }
  return bytes;
}


// reduce the ascii code to fit into the Code128 char table
function codeValue(chr1, chr2) {
  if (typeof chr2 === 'undefined') {
    return chr1 >= 32 ? chr1 - 32 : chr1 + 64;
  }

  return parseInt(String.fromCharCode(chr1) + String.fromCharCode(chr2), 10);
}

function codeSetAllowedFor(chr) {
  if (chr >= 48 && chr <= 57) {
    // 0-9
    return CODESET.ANY;
  }
  if (chr >= 32 && chr <= 95) {
    // 0-9 A-Z
    return CODESET.AB;
  }

  // if non printable
  return chr < 32 ? CODESET.A : CODESET.B;
}

function charCompatible(chr, codeset) {
  const csa = codeSetAllowedFor(chr);
  if (csa === CODESET.ANY) return true;
  // if we need to change from current
  if (csa === CODESET.AB) return true;
  if (csa === CODESET.A && codeset === CODESET.A) return true;
  if (csa === CODESET.B && codeset === CODESET.B) return true;
  return false;
}

function stringToCode128(text) {
  const barc = {
    currcs: CODESET.C,
  };

  const bytes = getBytes(text);
  // decide starting codeset
  let index = bytes[0] === CHAR_TILDE ? 1 : 0;

  const csa1 = bytes.length > 0 ? codeSetAllowedFor(bytes[index++]) : CODESET.AB;
  const csa2 = bytes.length > 0 ? codeSetAllowedFor(bytes[index++]) : CODESET.AB;

  function getBestStartSet(csa10, csa20) {
    // tries to figure out the best codeset
    // to start with to get the most compact code
    let vote = 0;
    vote += csa10 === CODESET.A ? 1 : 0;
    vote += csa10 === CODESET.B ? -1 : 0;
    vote += csa20 === CODESET.A ? 1 : 0;
    vote += csa20 === CODESET.B ? -1 : 0;
    // tie goes to B due to my own predudices
    return vote > 0 ? CODESET.A : CODESET.B;
  }

  function perhapsCodeC(bytes0, codeset) {
    for (let i = 0; i < bytes.length; i++) {
      const b = bytes0[i];
      if ((b < 48 || b > 57) && b !== CHAR_TILDE) { return codeset; }
    }
    return CODESET.C;
  }

  // chr1 is current byte
  // chr2 is the next byte to process. looks ahead.
  function codesForChar(chr1, chr2, currcs0) {
    const result = [];
    let shifter = -1;
    let currcs = currcs0;
    if (charCompatible(chr1, currcs)) {
      if (currcs === CODESET.C) {
        if (chr2 === -1) {
          shifter = SET_CODEB;
          currcs = CODESET.B;
        } else if ((chr2 !== -1) && !charCompatible(chr2, currcs)) {
          // need to check ahead as well
          if (charCompatible(chr2, CODESET.A)) {
            shifter = SET_CODEA;
            currcs = CODESET.A;
          } else {
            shifter = SET_CODEB;
            currcs = CODESET.B;
          }
        }
      }
    } else {
      /* eslint-disable no-lonely-if */
      // if there is a next char AND that next char is also not compatible
      if ((chr2 !== -1) && !charCompatible(chr2, currcs)) {
        // need to switch code sets
        switch (currcs) {
          case CODESET.A:
            shifter = SET_CODEB;
            currcs = CODESET.B;
            break;
          case CODESET.B:
            shifter = SET_CODEA;
            currcs = CODESET.A;
            break;
          default: break;
        }
      } else {
        // no need to shift code sets, a temporary SHIFT will suffice
        shifter = SET_SHIFT;
      }
      /* eslint-enable no-lonely-if */
    }

    // ok some type of shift is nessecary
    if (shifter !== -1) {
      result.push(shifter);
      result.push(codeValue(chr2));
    } else if (currcs === CODESET.C) {
      // include next as well
      result.push(codeValue(chr1, chr2));
    } else {
      result.push(codeValue(chr1));
    }
    barc.currcs = currcs;

    return result;
  }

  barc.currcs = getBestStartSet(csa1, csa2);
  barc.currcs = perhapsCodeC(bytes, barc.currcs);

  // if no codeset changes this will end up with bytes.length+3
  // start, checksum and stop
  /* eslint-disable no-array-constructor */
  let codes = new Array();

  switch (barc.currcs) {
    case CODESET.A:
      codes.push(SET_STARTA);
      break;
    case CODESET.B:
      codes.push(SET_STARTB);
      break;
    default:
      codes.push(SET_STARTC);
      break;
  }


  for (let i = 0; i < bytes.length; i++) {
    let b1 = bytes[i]; // get the first of a pair
    // should we translate/replace
    if (b1 in REPLACE_CODES) {
      codes.push(REPLACE_CODES[b1]);
      i++; // jump to next
      b1 = bytes[i];
    }

    // get the next in the pair if possible
    const b2 = bytes.length > (i + 1) ? bytes[i + 1] : -1;

    codes = codes.concat(codesForChar(b1, b2, barc.currcs));
    // code C takes 2 chars each time
    if (barc.currcs === CODESET.C) i++;
  }

  // calculate checksum according to Code 128 standards
  let checksum = codes[0];
  for (let weight = 1; weight < codes.length; weight++) {
    checksum += (weight * codes[weight]);
  }
  codes.push(checksum % 103);

  codes.push(SET_STOP);

  // encoding should now be complete
  return codes;
}

function code128({
  canvasId, text, x = 0, y = 0, width = 0, height = 0, usingIn, callback, fontColor = '#000000', backColor = '#ffffff',
}) {
  const ctx = (canvasId && wx.createCanvasContext(canvasId, usingIn));
  const $width = parseInt(width, 10);
  const $height = parseInt(height, 10);

  const codes = stringToCode128(text);

  // const g = new Graphics(ctx, $width, $height);
  // this._fillRect(0, 0, width, height, this.bg);
  ctx.setFillStyle(backColor);
  ctx.fillRect(x, y, $width, $height);

  const barWeight = $width / ((codes.length - 3) * 11 + 35);


  for (let i = 0, positionX = x, positionY = y; i < codes.length; i++) {
    const c = codes[i];
    // two bars at a time: 1 black and 1 white
    for (let bar = 0; bar < 8; bar += 2) {
      const barW = PATTERNS[c][bar] * barWeight;
      // var barH = height - y - this.border;
      const barH = height - positionY;
      const spcW = PATTERNS[c][bar + 1] * barWeight;

      // no need to draw if 0 width
      if (barW > 0) {
        // g._fillRect(positionX, positionY, barW, barH, fontColor);
        ctx.setFillStyle(fontColor);
        ctx.fillRect(positionX, positionY, barW, barH);
      }

      positionX += barW + spcW;
    }
  }
  ctx.draw(false, () => {
    setTimeout(() => {
      if (!(callback && typeof callback === 'function')) return;
      const options = {
        x: 0,
        y: 0,
        width,
        height,
        destWidth: width,
        destHeight: height,
        canvasId,
        success: callback,
      };
      wx.canvasToTempFilePath(options, usingIn);
    }, 800);
  });
}


export default code128;
